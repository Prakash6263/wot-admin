...
// Copy assets if source exists and destination doesn't already have them
if (fs.existsSync(assetsSource)) {
  try {
    // Check if assets already exist in public
    const filesInDest = fs.existsSync(assetsDest) ? fs.readdirSync(assetsDest).length : 0;
    
    if (filesInDest === 0) {
      copyDirRecursive(assetsSource, assetsDest);
      console.log('[Setup] Copied assets to public/assets');
    } else {
      console.log('[Setup] Assets already in public/assets');
    }
  } catch (err) {
